...
public class RecruitmentServiceTest {
	...
	/**
	 * Tests that a newly created person acctuly gets inserted in the database, by
	 * counting rows before and after a registration and evaluating if the numbers
	 * is greated after.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * Evaluates if a duplicate username gets rejected as a registration, evaluates
	 * that IllegalTransactionException gets thrown preventing someone from
	...
	public void registrationUsernameExistTest() throws Exception {
		register(dummyPerson.getFirstName(), dummyPerson.getLastName(), dummyPerson.getEmail(), dummyPerson.getSsn(),
				dummyPerson.getUsername(), dummyPerson.getPassword());
		register("funkar", "Det", "då@gmail.com", "9403521583", "tagen", "Detkanske");

	}

	/**
	 * Evaluates if a duplicate Email gets rejected as a registration, evaluates
	 * that IllegalTransactionException gets thrown preventing someone from
	 * registering with a Email that already exists.
	 * 
	 * @throws IllegalTransactionException
	 */
	@Test(expected = IllegalTransactionException.class)
	...
	/**
	 * Evaluates if a duplicate Ssn gets rejected as a registration, evaluates that
	 * IllegalTransactionException gets thrown preventing someone from registering
	 * with a Ssn that already exists.
	 * 
	 * @throws IllegalTransactionException
	 */
	@Test(expected = IllegalTransactionException.class)
	...
	private static void assertGreaterThan(long greater, long lesser, String message) {
		if (greater <= lesser) {
			fail((StringUtils.isNotBlank(message) ? message + " ==> " : "") + "Expected: a value greater than <"
					+ lesser + ">\n" + "But <" + greater + "> was " + (greater == lesser ? "equal to" : "less than")
					+ " <" + lesser + ">");
		}
